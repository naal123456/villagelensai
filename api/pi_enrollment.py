...
from .pi_bridge import PiBridgeError


ENROLLMENT_SCHEMA = "villagelens.pi-device-enrollment.v1"
ENROLLMENT_OBJECT = "pi-enrollments/v1/default.json"
...
class PiEnrollmentStore:
    """Persistent, single-device registry backed by the existing private bucket."""
    ...
    def _blob(self) -> Any:
        ...
        if bucket is None:
            raise PiBridgeError("PI_ENROLLMENT_STORAGE_UNAVAILABLE", 503)
    ...
    def _read(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            raise PiBridgeError("PI_ENROLLMENT_STORAGE_UNAVAILABLE", 503) from error
        ...
        if (
            ...
            raise PiBridgeError("PI_ENROLLMENT_RECORD_INVALID", 503)
        return value
    ...
    def enroll(self, device_profile_id: str, token: str) -> dict[str, str]:
        ...
        if existing is not None and existing["status"] == "active":
            raise PiBridgeError("PI_DEVICE_ALREADY_ENROLLED", 409)
        now = self._now()
        value = {
    ...
    def revoke(self, device_profile_id: str) -> dict[str, str]:
        ...
        if (
            ...
            raise PiBridgeError("PI_DEVICE_ENROLLMENT_NOT_FOUND", 404)
