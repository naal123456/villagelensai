...
class ApiTests(unittest.TestCase):
    ...
    def test_tester_page_is_served(self) -> None:
        ...
        self.assertIn(b'v2026-09-25.1', response.data)
        ...
        self.assertIn(b'function untrustedOcr', response.data)
        self.assertIn(b'declaredKannada&&telugu>=5', response.data)
        self.assertIn(b'if (!result||!box||untrustedOcr())', response.data)
        self.assertIn(b'if (untrustedOcr()) return []', response.data)
        self.assertIn(b'No reliable words are available.', response.data)
        self.assertIn(b'function acceptedStrongest', response.data)
        self.assertIn(b'function strongestStatus', response.data)
        self.assertIn(b'Sol finished, but the readers disagree.', response.data)
        self.assertIn(b"acceptedStrongest(strongest)?'best \xe2\x9c\x93':'review !'", response.data)
        ...
        self.assertIn(b"const automaticStages=[1,2,3],enabledStages=[1,2,3,4]", response.data)
        self.assertIn(b"const missing=automaticStages", response.data)
        self.assertIn(b"function requestStrongestReading", response.data)
        ...
        self.assertIn(b"cameraView.addEventListener('pointerup',cameraPointerUp)", response.data)
        ...
        self.assertIn(b"?'best \xe2\x9c\x93'", response.data)
        ...
        foreground_flow = response.data.split(b"document.addEventListener('visibilitychange'", 1)[1].split(
            b"window.addEventListener('pageshow'", 1,
        )[0]
        self.assertLess(
            foreground_flow.index(b'if (audioContext) discardAudioContext()'),
            foreground_flow.index(b'await ensureCurrentApp()'),
        )
        ...
        self.assertIn(b"play(readingWords(),'play-words','word')", response.data)
        self.assertIn(b"play(readingLines(),'play-sentences','line')", response.data)
        ...
        self.assertIn("ಉಳಿಸಿದ ಚಿತ್ರ ನೋಡಲು ಬಾಣ ಒತ್ತಿ".encode(), response.data)
        self.assertIn("ಅ ಆ ಇ".encode(), response.data)

    def test_health_reports_model_cascade(self) -> None:
        response = self.client.get("/health")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["reader_models"], {
            "stage_2": "gpt-5.6-luna",
            "stage_3": "gpt-5.6-terra",
            "stage_4": "gpt-5.6-sol",
        })
        self.assertEqual(response.get_json()["stage_4"], "manual")
        self.assertEqual(response.get_json()["app_version"], "2026-09-25.1")
        page = self.client.get("/a/?tester=a3")
        self.addCleanup(page.close)
        self.assertIn(b'v2026-09-25.1', page.data)
    ...
    def test_old_sol_and_astra_evidence_is_not_current(self) -> None:
        ...
        self.assertFalse(_current_reader_stage({
            "reader": "vision_language", "model": "gpt-6-astra",
            "analysis_version": "astra-review-v2",
        }, 4))
        self.assertTrue(_current_reader_stage({
            "reader": "vision_language", "model": "gpt-5.6-luna",
    ...
    def test_unified_reader_has_language_toggle_and_silent_welcome(self) -> None:
        ...
        self.assertIn(b"location.assign(canonicalUrl(next,true))", response.data)
        self.assertIn(b"initialTester==='a3'", response.data)
        self.assertIn(b"testerId==='a3'&&languageParameter==='kn'&&!pendingView", response.data)
        self.assertIn(b"location.replace('/a/?tester=a3'); return", response.data)
    ...
    def test_a3_stale_kannada_url_redirects_to_english_home(self) -> None:
        ...
        self.assertEqual(stale.headers["Location"], "/a/?tester=a3&lang=en")
        self.assertEqual(active_switch.status_code, 200)
    ...
    def test_installable_app_assets_and_share_target_are_served(self) -> None:
        ...
        self.assertIn(b"const APP_VERSION = '2026-09-25.1'", worker.data)
    ...
    def test_stage_two_uses_luna_standard_first_pass(self, reader: object, store: object) -> None:
        reader.return_value = {
            "schema": "villagelens.reader.v1", "stage": 2,
            "reader": "vision_language", "analysis_version": "luna-compact-v1",
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["analysis_version"], "luna-compact-v1")
        self.assertEqual(reader.call_args.kwargs["stage"], 2)
        self.assertEqual(reader.call_args.kwargs["model"], "gpt-5.6-luna")
        self.assertEqual(reader.call_args.kwargs["service_tier"], "default")
    ...
    def test_quick_reader_uses_small_low_detail_contract(self, provider: object) -> None:
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            value = _openai_reader(
                ...
                analysis_version="luna-compact-v1", quick=True,
            )
    ...
    def test_quick_reader_can_return_english_for_any_source_script(self, provider: object) -> None:
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "test-key"}):
            value = _openai_reader(
                image_bytes(), "image/jpeg", 320, 120, stage=2,
                model="gpt-5.6-luna", service_tier="default",
                analysis_version="luna-compact-v1", quick=True, output_language="en",
            )
    ...
    def test_stored_processing_reuses_all_existing_stages(
        ...
        values = {
            ...
            f"captures/{capture_id}/stage-2.json": {
                "stage": 2, "reader": "vision_language", "model": "gpt-5.6-luna",
                "analysis_version": "luna-compact-v1", "summary_kn": "ಪುಸ್ತಕ",
            },
    ...
    def test_stored_processing_leaves_stage_four_on_demand(self, process: object) -> None:
        ...
        with patch("api.app._storage_bucket", return_value=MagicMock()), \
                patch("api.app._authorized_capture"):
            stages, errors = _process_stored_capture(capture_id, "a3")
    ...
    @patch("api.app._process_stored_stage")
    def test_stage_four_endpoint_runs_only_when_requested(self, process: object) -> None:
        capture_id = "f" * 32
        process.return_value = {"stage": 4, "model": "gpt-5.6-sol"}
        response = self.client.post(f"/api/captures/{capture_id}/process/4")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json()["result"]["model"], "gpt-5.6-sol")
        process.assert_called_once_with(capture_id, "", 4)
    ...
    def test_usage_events_accept_only_privacy_safe_aggregates(self) -> None:
        with self.assertLogs("api.app", level="INFO") as logs:
            response = self.client.post("/api/events", json={"app_version": "2026.09.15.6<script>", "events": [
        ...
        self.assertIn('"app_version":"2026.09.15.6script"', joined)
